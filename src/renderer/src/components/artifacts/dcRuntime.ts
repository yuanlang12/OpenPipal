...
const SUPPORT_BOOT_CHAIN = 'loadReactUmd().then(init).catch((err) => {'
const SUPPORT_BOOT_CHAIN_WITH_DEPENDENCIES =
  'loadReactUmd().then(() => typeof window.__openpipalWaitForDcDependencies === "function" ? window.__openpipalWaitForDcDependencies() : undefined).then(init).catch((err) => {'
const SUPPORT_SELF_FETCH = 'fetch(location.href).then('
const SUPPORT_SELF_FETCH_SRC_DOC_SAFE = 'if (location.protocol !== "about:") fetch(location.href).then('
...
export function inlineDcRuntime(html: string): string {
  // 官方 runtime 会再次 fetch(location.href) 读取原始 dc；srcdoc 的 location 是
  // about:srcdoc，Chromium 会直接报红。宿主已有完整源码，跳过这次无意义自取即可。
  const supportForSrcDoc = supportJsRaw.includes(SUPPORT_SELF_FETCH)
    ? supportJsRaw.replace(SUPPORT_SELF_FETCH, SUPPORT_SELF_FETCH_SRC_DOC_SAFE)
    : supportJsRaw
  // support.js 会异步加载 React，然后立刻 boot <x-dc>。会话 JSX sidecar 也要等 React/Stage
  // 才能注册全局组件；不加这个宿主钩子时，support 的 Promise microtask 会抢在 sidecar 的
  // 30ms poll 前 boot，x-import 读到 undefined（React #130）。钩子不存在时保持官方行为。
  const supportWithDependencyGate = supportForSrcDoc.includes(SUPPORT_BOOT_CHAIN)
    ? supportForSrcDoc.replace(SUPPORT_BOOT_CHAIN, SUPPORT_BOOT_CHAIN_WITH_DEPENDENCIES)
    : supportForSrcDoc
  const inline = `<script>${supportWithDependencyGate}</script>`
 ...
 */
const KNOWN_SIBLINGS: { fromRe: RegExp; source: string }[] = [
  { fromRe: /\s(?:from|src|import)="\.\/deck-stage\.js"/g, source: deckStageRaw },
  { fromRe: /\s(?:from|src|import)="\.\/animations\.(?:jsx|js)"/g, source: animationsCompiledRaw },
  { fromRe: /\s(?:from|src|import)="\.\/ios-frame\.(?:jsx|js)"/g, source: iosFrameCompiledRaw },
  { fromRe: /\s(?:from|src|import)="\.\/android-frame\.(?:jsx|js)"/g, source: androidFrameCompiledRaw },
  // image-slot.js：doc-page 同型的零依赖自注册 Web Component（helmet <script src> 加载）
  { fromRe: /\s(?:from|src|import)="\.\/image-slot\.js"/g, source: imageSlotRaw },
  // doc-page.js：零依赖自注册 Web Component（无需 React/Babel/poll 包装）。源码 JSDoc 含 </script 字面量，
  // 内联时统一走下方 .replace(/<\/script/g,…) 转义路径。
  { fromRe: /\s(?:from|src|import)="\.\/doc-page\.js"/g, source: docPageRaw },
]
