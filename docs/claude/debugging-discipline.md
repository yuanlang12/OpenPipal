...
## 判据静默恒假：先看"解析出多少条"，再怀疑模型（2026-08-26 · SWE-bench 对照实验废掉 40% 样本）

**现象**：40 道题的配对对照里，requests 和 xarray 共 12 道**两条臂全灭**，而这些题全部通过了标准答案闸门。
手跑复现不了：同一棵树、同一个补丁，手动跑就是绿的。

**根因**：Playwright 的 workerHost 给每个 worker 进程硬塞 `FORCE_COLOR=1`
（`node_modules/playwright/lib/runner/workerHost.js`）。判分脚本用 `dict(os.environ, ...)`
原样继承，`execFileSync` 一路传到 pytest；pytest≥7 认这个变量，于是 `-rA` 短摘要的每一行
都以 ANSI 转义序列开头，而 SWE-bench 官方 parser 判的是 `line.startswith("PASSED")`——
一条都认不出来，`parsed=0`，**所有测试静静地记成未过，不报任何错**。

**为什么查了很久**：症状长得完全像"模型能力不行"。两条臂拿到逐条相同的判负结果，
时长正常、工具调用正常、diff 看着也合理。真正戳破它的是三条硬事实的组合——

1. 某道题的标准答案就是一行 `variable_attrs[0]` → `dict(variable_attrs[0])`，
   而**两条臂记录的 diffstat 与标准答案逐字节相同**，却都判负；
2. A 臂和 B 臂的 `p2p_new_fail` 列表**逐条相同**，哪怕两条臂的 diff 完全不同
   ——失败与补丁内容无关，就不可能是模型造成的；
3. 判分结果里 `p2p_new_fail` 被截断成 6 条，掩盖了"**全灭**"和"坏了 6 条"的区别。
   把总数打出来才看见是整套没跑起来。

**教训**：

- **判分/校验类子进程的环境必须显式构造，不能原样继承调用方。** 判据是"确定性归代码"的那一半，
  它的输入不能是"上游碰巧带进来的那些"。同类变量还有 `CLICOLOR_FORCE`、`TERM`、`COLUMNS`、`CI`、`LANG`。
- **静默判负比崩溃危险得多。** 崩溃会被看见；静默恒假会被当成"模型能力"写进报告。
  任何"解析日志得出结论"的环节，都要把**解析出多少条**一起记下来并设下界——`parsed=0` 必须是硬错误，
  不能悄悄降级成"全部未通过"。
- **报数不要只留截断后的样例**，把总数一起留下。截断的清单会让"全灭"读起来像"坏了几条"。
- **手跑复现不了 ≠ 不存在**。手跑的 shell 和被测进程的 shell 环境不同，
  这本身就是最值得查的一条线索——先 `diff` 两边的环境，再怀疑别的。
- **留证据要留到能归因为止**。这批只记了 `diffstat` 摘要，没记 agent 的实际 patch，
  也没记 `parsed`，于是"模型做错了"和"评测台坏了"事后区分不开——只能重跑。

**同一批还挖出第二个**：test_patch 打不上时兜底走 `git checkout -- .` 整棵树还原再打，
把 agent 的源码修复也一起扔了，F2P 必然全红。官方 harness 的做法是只 `git checkout <base> -- <测试文件>`。
这条路**恰好在 agent 动过测试文件时才触发**——agent 越是老老实实先写测试再改代码越容易被判负，
判据和好行为反相关，是最坏的一种 bug。
