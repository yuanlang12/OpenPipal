...
describe('secondary renderer surfaces i18n', () => {
  it('serves matching English and Chinese resources', async () => {
    ...
    expect(english.t('tasks.actions.create')).toBe('New automation')
    expect(english.t('tasks.editor.namePlaceholder')).toBe('Automation name')
    expect(english.t('tasks.templates.title')).toBe('Automation templates')
    ...
    expect(chinese.t('tasks.actions.create')).toBe('新建自动化')
    expect(chinese.t('tasks.editor.namePlaceholder')).toBe('自动化名称')
    expect(chinese.t('tasks.templates.title')).toBe('自动化模板')
