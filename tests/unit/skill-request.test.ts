/**
 * 技能选择 = 单条消息内的强调（不再是会话级白名单），输入侧是 `/` 快捷指令 token。
 * 覆盖：
 * - composeSkillRequest（兜底句式 + 标签格式的单一出处）
 * - findSkillMentions（只认已知名单、最长匹配、左右边界、中文/空格名）
...
describe('findSkillMentions', () => {
  it('只认已知名单；贴在词/路径中间的 / 不算 token', () => {
    expect(findSkillMentions('/nope 帮我看看', KNOWN)).toEqual([])
    expect(findSkillMentions('看下 src/pdf 这个目录', KNOWN)).toEqual([])
    expect(findSkillMentions('打开 https://x.com/pdf', KNOWN)).toEqual([])
  })
